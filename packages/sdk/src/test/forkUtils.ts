import {
    Abi,
    Address,
    CallParameters,
    type Chain,
    createClient,
    createPublicClient,
    createTestClient,
    createWalletClient,
    formatTransactionRequest,
    Hex,
    http
} from 'viem'
import { baseSepolia } from 'viem/chains'
import axios from 'axios';

export const FORK_BLOCK_NUMBER = 11741439
export const FORK_URL = 'https://sepolia.base.org'

export const rollupAnvilChain = {
    ...baseSepolia,
    id: 84532,
    name: 'Rollup Localhost',
    nativeCurrency: {
        decimals: 18,
        name: 'Ether',
        symbol: 'ETH',
    },
    rpcUrls: {
        default: {
            http: [`http://127.0.0.1:8545`],
            webSocket: [`ws://127.0.0.1:8545`],
        },
        public: {
            http: [`http://127.0.0.1:8545`],
            webSocket: [`ws://127.0.0.1:8545`],
        },
    },
} as const satisfies Chain

export const testClient = createTestClient({
    chain: rollupAnvilChain,
    mode: 'anvil',
    transport: http(),
})

export const publicClient = createPublicClient({
    chain: rollupAnvilChain as Chain,
    transport: http(),
})

export const walletClient = (account: Address) => {
    return createWalletClient({
        chain: rollupAnvilChain,
        transport: http(),
        account,
    })
}

export const debugClient = createClient({
    chain: rollupAnvilChain,
    transport: http()
}).extend((client: any) => ({
    // ...
    async traceCall(args: CallParameters) {
        return client.request({
            method: 'debug_traceCall',
            params: [formatTransactionRequest(args), 'latest', {}]
        })
    },
    // ...
}))

export const ANVIL_ACCOUNTS_PK: Hex[] = [
    '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80',
    '0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d',
    '0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a',
    '0x7c852118294e51e653712a81e05800f419141751be58f605c371e15141b007a6',
    '0x47e179ec197488593b187f80a00eb0da91f1b9d0b13f8733639f19c30a34926a',
    '0x8b3a350cf5c34c9194ca85829a2df0ec3153be0318b5e2d3348e872092edffba',
    '0x92db14e403b83dfe3df233f83dfa3a0d7096f21ca9b0d6d6b8d88b2b4ec1564e',
    '0x4bbbf85ce3377467afe5d46f804f221813b2bb87f24d81f60f1fcdbf7cbf4356',
    '0xdbda1821b80551c9d65939329250298aa3472ba22feea921c0cf5d620ea67b97',
    '0x2a871d0798f97d79848a013d4936a73bf4cc922c825d33c1cf7073dff6d409c6',
]

export const ACCOUNTS = [
    '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266',
    '0x70997970C51812dc3A010C7d01b50e0d17dc79C8',
    '0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC',
    '0x90F79bf6EB2c4f870365E785982E1f101E93b906',
    '0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65',
    '0x9965507D1a55bcC2695C58ba16FB37d819B0A4dc',
    '0x976EA74026E726554dB657fA54763abd0C3a0aa9',
    '0x14dC79964da2C08b23698B3D3cc7Ca32193d9955',
    '0x23618e81E3f5cdF7f54C3d65f7FBc0aBf5B21E8f',
    '0xa0Ee7A142d267C1f36714E4a8F75612F20a79720',
] as const

// Named accounts
export const [ALICE, BOB, CHARLIE, CHANNEL_TREASURY] = ACCOUNTS
export const [ALICE_PK, BOB_PK] = ANVIL_ACCOUNTS_PK

export const baseSepoliaWETH = '0x4200000000000000000000000000000000000006'

export const GENERIC_ERC20_ABI = [
    {
        "constant": false,
        "inputs": [{ "name": "payableAmount", "type": "uint256" }],
        "name": "deposit",
        "outputs": [],
        "payable": true,
        "stateMutability": "payable",
        "type": "function"
    },
    {
        "constant": true,
        "inputs": [{ "name": "account", "type": "address" }],
        "name": "balanceOf",
        "outputs": [{ "name": "", "type": "uint256" }],
        "payable": false,
        "stateMutability": "view",
        "type": "function"
    },
    {
        "constant": false,
        "inputs": [{ "internalType": "address", "name": "guy", "type": "address" }, { "internalType": "uint256", "name": "wad", "type": "uint256" }],
        "name": "approve",
        "outputs": [{ "internalType": "bool", "name": "", "type": "bool" }],
        "payable": false,
        "stateMutability": "nonpayable",
        "type": "function"
    },
    {
        "constant": true,
        "inputs": [{ "internalType": "address", "name": "", "type": "address" }, { "internalType": "address", "name": "", "type": "address" }],
        "name": "allowance",
        "outputs": [{ "internalType": "uint256", "name": "", "type": "uint256" }],
        "payable": false,
        "stateMutability": "view",
        "type": "function"
    }
] as Abi;

export const balanceOfERC20 = async (token: Address, user: Address) => {
    await publicClient.readContract({
        address: token,
        abi: GENERIC_ERC20_ABI,
        functionName: 'balanceOf',
        args: [user] // Replace with actual account address
    });
}

export const allowanceOfERC20 = async (token: Address, contract: Address, user: Address) => {
    await publicClient.readContract({
        address: token,
        abi: GENERIC_ERC20_ABI,
        functionName: 'allowance',
        args: [user, contract]
    })

}

export const mintERC20 = async (token: Address, user: Address, amount: bigint) => {
    await walletClient(user).writeContract({
        address: token,
        abi: GENERIC_ERC20_ABI,
        functionName: 'deposit',
        args: [amount],
        value: amount
    })
}

export const setERC20Approval = async (token: Address, contract: Address, user: Address, amount: bigint) => {
    await walletClient(user).writeContract({
        address: token,
        abi: GENERIC_ERC20_ABI,
        functionName: 'approve',
        args: [contract, amount]
    })
}

export const getBlock = async () => {
    // get the current block from forked instance
    const requestData = {
        jsonrpc: "2.0",
        method: "eth_blockNumber",
        id: 1
    };

    try {
        return await axios.post('http://127.0.0.1:8545', requestData).then(res => Number(res.data.result))
    } catch (error) {
        console.error('Failed to get block', error);
        throw error; // Rethrow error to fail the test if reverting fails
    }
}

export const resetBlockchainState = async () => {
    const requestData = {
        jsonrpc: "2.0",
        method: "anvil_reset",
        params: [
            {
                forking: {
                    blockNumber: 'latest'
                }
            }
        ],
        id: 1
    };

    try {
        const response = await axios.post('http://127.0.0.1:8545', requestData);
    } catch (error) {
        console.error('Failed to revert fork to block', error);
        throw error; // Rethrow error to fail the test if reverting fails
    }
};


export const evmMine = async () => {
    const requestData = {
        jsonrpc: "2.0",
        method: "evm_mine",
        id: 1
    };

    try {
        const response = await axios.post('http://127.0.0.1:8545', requestData);
    } catch (error) {
        throw error;
    }
}

export const increaseEvmTime = async (amount: number) => {
    const requestData = {
        jsonrpc: "2.0",
        method: "evm_increaseTime",
        params: [amount],
        id: 1
    };

    try {
        const response = await axios.post('http://127.0.0.1:8545', requestData);
        await evmMine();

    } catch (error) {
        throw error;
    }
};