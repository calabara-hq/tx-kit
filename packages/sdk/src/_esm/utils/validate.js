...
export const validateFeePercentage = (fee) => {
    if (fee < 0 || fee > 100) {
...
export const validateSetFeeInputs = (args) => {
    ...
    if (args.feeArgs) {
        ...
        totalAllocation = roundToDecimals(totalAllocation, MAX_PRECISION_DECIMALS);
        if (totalAllocation !== 100)
            throw new InvalidArgumentError(`Fee percentages must add up to 100. The current total is ${totalAllocation}`);
    }
};
export const validateSetLogicInputs = (inputs) => {
    const data = {
        targets: [],
        signatures: [],
        datas: [],
    ...
    if (!allArraysEqualLength) {
        throw new InvalidArgumentError('Invalid logic inputs. Each logic element must have shape [interactionPower] if result of [target, signature, data] [gt/lt/eq] [literalOperand]');
    }
};
export const validateInfiniteTransportLayer = (transportLayer) => {
...
export const validateFiniteTransportLayer = (transportLayer) => {
...
export const validateSetupActions = (actions) => {
...
export const validateInfiniteChannelInputs = (inputs) => {
...
export const validateFiniteChannelInputs = (inputs) => {
    validateAddress(inputs.defaultAdmin);
    inputs.managers.forEach(validateAddress);
    validateSetupActions(inputs.setupActions);
    validateFiniteTransportLayer(inputs.transportLayer);
};
export const validateCreateTokenInputs = (inputs) => {
...
export const validateMintTokenBatchInputs = (inputs) => {
    validateAddress(inputs.channelAddress);
...
export const validateSponsorTokenInputs = async (inputs) => {
    validateAddress(inputs.channelAddress);
    validateAddress(inputs.to);
    validateAddress(inputs.mintReferral);
...
export const validateWithdrawRewardsInputs = (inputs) => {
    validateAddress(inputs.channelAddress);
    validateAddress(inputs.to);
...
export const validateApproveERC20Inputs = (inputs) => {
